...
def test_apply_bird_with_mock():
    ...
    with patch("src.mine_and_dedupe.handle_two_pairs") as mock_handle:
        ...
        assert all("name_1" in r for r in results)

def test_boolean_bird_heuristic():
    two_pairs = {
    ...
    assert result["c1"] >= 0.0  # just ensure similarity key exists
